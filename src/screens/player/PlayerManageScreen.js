...
export default class PlayerManageScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            players: [],
    ...
    _movePlayer = () => {
        ...
        let { currentTeam, club, players } =  this.props.navigation.state.params;
        ...
        if (selectedPlayers === undefined || selectedPlayers.length == 0) {
            ...
            navigate('SelectTeam', { selectedPlayers, currentTeam:currentTeam||null, club });
        } else {
            const { navigate } = this.props.navigation;
            let selectedPlayersObj = players.filter(function (player) {
                return selectedPlayers.includes(player.id);
            });
            navigate('ChoosePlayersTeam', { selectedPlayersObj, currentTeam:currentTeam||null, club });
        }
    }
    _addPlayer = () => {
        const { checked } = this.state;
        const selectedPlayers = [];
        let { club, currentTeam } =  this.props.navigation.state.params;
        checked.forEach((chk, playerId) => {
            playerId !== 0 && chk && selectedPlayers.push(playerId);
        });
        if (selectedPlayers === undefined || selectedPlayers.length == 0) {
            ...
            const { navigate } = this.props.navigation;
            navigate('SelectTeam', { selectedPlayers, type: 'add', currentTeam:currentTeam||null, club });
        }
    }
    ...
    render() {
        ...
        const { players, moveDisabled, addDisabled } = this.props.navigation.state.params;
        ...
        return (
            <View style={styles.mainWrapper}>
                ...
                <ManageFooter navigation={navigation} movePlayer={!moveDisabled?this._movePlayer:null} addPlayer={!addDisabled?this._addPlayer:null} deleteItems={this._deleteItems}/>
            </View>
        );
    }
